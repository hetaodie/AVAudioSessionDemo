...
- (BOOL)setActive:(BOOL)active
  ...
  if (success) {
    ...
    NSLog(@"Failed to setActive:%d. Error: %@",
  ...
  NSLog(@"Number of current activations: %d", _activationCount);
  return success;
}
...
- (void)handleApplicationDidBecomeActive:(NSNotification *)notification {
  if (self.isInterrupted) {
    NSLog(@"Application became active after an interruption. Treating as interruption end.");
    self.isInterrupted = NO;
    [self updateAudioSessionAfterEvent];
...
- (BOOL)configureWebRTCSession:(NSError **)outError {
  ...
  NSLog(@"Configuring audio session for WebRTC.");
  ...
  if (![self setConfiguration:webRTCConfig active:YES error:&error]) {
    NSLog(@"Failed to set WebRTC audio configuration: %@",
...
- (BOOL)unconfigureWebRTCSession:(NSError **)outError {
  ...
  NSLog(@"Unconfiguring audio session for WebRTC.");
